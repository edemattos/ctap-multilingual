...
public class FeatureSetManager extends Composite {
	...
	@UiField Anchor selectDutchRightPanel;
	@UiField Anchor selectDutch;
	...
	private void showAvailableFeatures() {
		...
		// default to 10 features
		availableFeatureCellTable.setVisibleRange(0, 10);
		// default to show all features
...
//		selectedFeatureCellTable.setVisibleRange(0, selectedFeatureCellTable.getRowCount());
	...
	private void setWidgetStyle() {
		...
		selectDutch.getElement().getStyle().setCursor(Cursor.POINTER);
...
//		refreshSelectedFeatureList();
	...
	@UiHandler("selectDutch")
	void onSelectDutchClick(ClickEvent e) {
		for(AnalysisEngine complexityFeature : selectedFeatureCellTable.getVisibleItems()) {
			selectedFeatureListSelectionModel.setSelected(complexityFeature, 
					complexityFeature.getSupportedLanguages().contains("nl"));
		}
	}
	@UiHandler("selectDutchRightPanel")
	void onSelectDutchRightPanelClick(ClickEvent e) {
		for(AnalysisEngine complexityFeature : availableFeatureCellTable.getVisibleItems()) {
			availableFeatureSelectionModel.setSelected(complexityFeature, 
					complexityFeature.getSupportedLanguages().contains("nl"));
		}
	...
	@UiHandler("selectPortuguese")
	void onSelectPortugueseClick(ClickEvent e) {
		for(AnalysisEngine complexityFeature : selectedFeatureCellTable.getVisibleItems()) {
			selectedFeatureListSelectionModel.setSelected(complexityFeature,
					complexityFeature.getSupportedLanguages().contains("pt"));
		}
	}
	@UiHandler("selectPortugueseRightPanel")
	void onSelectPortugueseRightPanelClick(ClickEvent e) {
		for(AnalysisEngine complexityFeature : availableFeatureCellTable.getVisibleItems()) {
			availableFeatureSelectionModel.setSelected(complexityFeature,
					complexityFeature.getSupportedLanguages().contains("pt"));
		}
